...
`vercel.json` には1日1回の cron 設定を入れています。`GET /api/reminders/digest` が Supabase から当日の未購入リマインド対象を収集し、同日二重送信を防いで Resend で送信します。`RESEND_API_KEY` または `REMINDER_FROM_EMAIL` が未設定の場合は preview のみ返します。
