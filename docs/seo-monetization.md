...
- `/llms.txt`
- `/about`
- 広告枠が有効な公開ページだけでGoogle AdSenseスクリプトを遅延読み込み
- トップ・説明・ログイン・規約・プライバシー・問い合わせページの広告枠
- Cloudflare実行時の環境変数から広告IDを取得する `/api/ads/config`
- Cloudflare実行時の環境変数から動的に返す `/ads.txt`
...
6. `Privacy & messaging` で欧州規制向けメッセージを作成し、Google認定CMPによる同意管理を有効にする。
7. `npm run cf:deploy` で再デプロイする。

広告IDはブラウザへ公開される識別子であり、秘密鍵ではありません。設定値はCloudflare Workerの実行時APIから配信するため、ローカルの`.env.local`へ広告IDを保存しなくても本番広告を有効化できます。

## 公開後の確認

1. `https://app.shareshopi.workers.dev/ads.txt` がHTTP 200かつ正しいサイト運営者IDを返すことを確認する。
2. トップ・説明・ログイン・法務ページでのみ広告リクエストが発生することを確認する。
3. `/lists/` と設定画面には広告が表示されないことを確認する。
4. AdSenseの `Privacy & messaging` で同意メッセージの公開状態を確認する。
5. AdSenseのポリシーセンターと広告配信状況を定期的に確認する。
...
- 広告は公開ページだけに表示し、買い物リストや設定などの操作画面には表示しません。
